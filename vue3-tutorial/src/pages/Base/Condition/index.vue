<template>
  ...
  <left-title title="<random>v-if</random> <blue>vs</blue> <red>v-show</red>">
    <div>
      v-if
      ...
      display 属性会被切换。 总的来说，v-if 有更高的切换开销，而 v-show
  ...
  <left-title
    title="当 v-if 和 v-for 同时存在于一个元素上的时候，v-if 会首先被执行。请查看列表渲染指南获取更多细节。"
  >
    <div>注意</div>
  </left-title>
</template>
